class TemporaryInventoryBatchNumber < ApplicationRecord
	  ...
	  def update_next_stock
	  	...
	  	if next_periode <= DateTime.now.strftime("%Y%m")
		  	if self.product_batch_number.present?
		  		next_inventory = TemporaryInventoryBatchNumber.find_by(:company_profile_id=> self.company_profile_id, :product_batch_number_id=> self.product_batch_number_id, :periode=> next_periode)
		  		if next_inventory.present?
			  		...
			  		next_inventory = TemporaryInventoryBatchNumber.new({
		  		...
		  		next_inventory = TemporaryInventoryBatchNumber.find_by(:company_profile_id=> self.company_profile_id, :material_batch_number_id=> self.material_batch_number_id, :periode=> next_periode)
		  		if next_inventory.present?
		  			end_stock = ((begin_stock.to_f+next_inventory.trans_in.to_f)-next_inventory.trans_out.to_f)
		  		...
		  		next_inventory = TemporaryInventoryBatchNumber.find_by(:company_profile_id=> self.company_profile_id, :general_batch_number_id=> self.general_batch_number_id, :periode=> next_periode)
		  		if next_inventory.present?
		  			end_stock = ((begin_stock.to_f+next_inventory.trans_in.to_f)-next_inventory.trans_out.to_f)
		  			next_inventory.update({:begin_stock=> begin_stock, :end_stock=> end_stock})
		  		end
		  	elsif self.consumable_batch_number.present?
		  		next_inventory = TemporaryInventoryBatchNumber.find_by(:company_profile_id=> self.company_profile_id, :consumable_batch_number_id=> self.consumable_batch_number_id, :periode=> next_periode)
		  		if next_inventory.present?
		  			end_stock = ((begin_stock.to_f+next_inventory.trans_in.to_f)-next_inventory.trans_out.to_f)
		  		...
		  		next_inventory = TemporaryInventoryBatchNumber.find_by(:company_profile_id=> self.company_profile_id, :equipment_batch_number_id=> self.equipment_batch_number_id, :periode=> next_periode)
		  		if next_inventory.present?
		  			end_stock = ((begin_stock.to_f+next_inventory.trans_in.to_f)-next_inventory.trans_out.to_f)
