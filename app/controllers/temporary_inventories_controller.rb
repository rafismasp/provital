class TemporaryInventoriesController < ApplicationController
  ...
  def index
    ...
    inventories = TemporaryInventory.where(:company_profile_id=> current_user.company_profile_id, :periode=> session[:periode]).where("trans_in > 0 or trans_out > 0 or end_stock > 0")
    ...
    case params[:partial]
    when 'stock_card'      
      @inventory = TemporaryInventory.find_by(:periode=> params[:periode], "#{params[:select_inventory_kind]}".to_sym=> params[:part])
      if @inventory.present?
        @inventory_logs = TemporaryInventoryLog.where(:temporary_inventory_id=> @inventory.id, :status=> 'active')
      end
    end
  ...
  private
    ...
    def set_inventory
      @inventory = TemporaryInventory.find(params[:id])
      @inventory_logs = TemporaryInventoryLog.where(:temporary_inventory_id=> params[:id], :status=> 'active')
    end
